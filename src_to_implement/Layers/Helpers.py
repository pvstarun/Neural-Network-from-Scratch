...
def gradient_check(layers, input_tensor, label_tensor): #checks whether the gradients during backpropagation are correct by comparing them to numerical approximations of the gradients f(x+eps)-(f(x-eps))/2eps
    epsilon = 1e-5
...
class RandomData:
    ...
    def next(self):
        input_tensor = np.random.random([self.batch_size, self.input_size]) # generate random input tensor of shape (batch_size, input_size)
...
class IrisData:
    def __init__(self, batch_size):
        ...
        self._input_tensor /= np.abs(self._input_tensor).max() # normalize the input tensor to have values between 0 and 1
    ...
    def _forward_idx_iterator(self):
        ...
        idx = np.arange(self.split) # create an array of indices from 0 to split-1
        while True:
            this_idx = np.random.choice(idx, self.split, replace=False) # randomly shuffle the indices for each epoch
            for i in range(num_iterations):
                yield this_idx[i * self.batch_size:(i + 1) * self.batch_size] # yield a batch of indices for each iteration
